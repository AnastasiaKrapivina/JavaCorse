...
*Ожидаемый результат:* Система выдает информационное сообщение о том, что операция одобрена банком.<br>
...
*Ожидаемый результат:* Система выдает информационное сообщение о том, что операция одобрена банком.<br>
5. **Тест 5 "Проверка ответа БД при оплате APPROVED картой":**<br>
"Номер карты"(APPROVED карта), "Месяц"(10), "Год"(24), "Владелец"(IVAN IVANOV), "CVC/CVV"(321). Нажать кнопку "Продолжить". <br>
*Ожидаемый результат:* В базе данных появляется информация о запросе, IP карты и "status":"APPROVED".<br>

**2. Негативные кейсы:**<br>
***Общее описание данных для негативных сценариев:***<br>
- Поле "Номер карты":  специальные номера карт, которые даны для тестирования (DECLINED карта — 5555 6666 7777 8888) <br>
- Поле "Месяц": Формат ввода - цифровой(XX). 
- Поле "Год": Формат ввода - две последние цифры(ХХ). <br>
  Значения полей "Месяц" и "Год" должны быть ранее, чем текущее значение даты.
- Поле "Владелец": Значения (Имя Фамилия) латинскими строчными буквами, на кириллице, длина менее 4 символа, цифрами.<br>
- Поле "CVC/CVV": состоит из менее, чем трёх цифр и буквы <br>

6. **Тест 6 "Проверка ответа БД при попытке оплаты DECLINED картой":**<br>
"Номер карты"(DECLINED карта), "Месяц"(10), "Год"(24), "Владелец"(IVAN IVANOV), "CVC/CVV"(321). Нажать кнопку "Продолжить". <br>
*Ожидаемый результат:* В базе данных появляется информация о запросе, IP карты и "status":"DECLINED".<br>
7. **Тест 7 "Отправка незаполненных полей формы":** <br>
...
8. **Тест 8 "Отправка незаполненного поля "Номер карты" ":**<br> 
...
9. **Тест 9 "Отправка незаполненного поля "Месяц"":** <br>
...
10. **Тест 10 "Отправка незаполненного поля "Год"":**<br>
...
11. **Тест 11 "Отправка незаполненного поля "Владелец"":** <br>
...
12. **Тест 12 "Отправка незаполненного поля "CVC/CVV"":** <br>
...
13. **Тест 13 "Оплата DECLINED картой":**<br>
Поле "Номер карты"(DECLINED карта), в поля "Месяц", "Год", "Владелец", "CVC/CVV". Нажать кнопку "Продолжить". <br>
*Ожидаемый результат:* Система выдает информационное сообщение об ошибке, операция не одобрена банком.<br>
14. **Тест 14 "Оплата просроченной картой, срок действия истек месяц назад":** <br>
...
15. **Тест 15 "Ввод недопустимого значения в поле "Месяц"":** <br>
...
16. **Тест 16 "Оплата просроченной картой, срок действия истек год назад":**<br>
...
17. **Тест 17 "Ввод недопустимого значения в поле "Год"":** <br>
...
18. **Тест 18  "Ввод значения в поле "Владелец" на кириллице":**<br>
...
23. **Тест 23 "Ввод значения в поле "CVC/CVV" длинной менее, чем допустимое":** <br>
...
2. POST запрос приложения "Путешествие дня" формы "Оплата по карте"(http://localhost:8080/api/v1/pay)
3. POST запрос приложения "Путешествие дня" формы "Кредит по данным карты"(http://localhost:8080/api/v1/credit)
...
4. Невозможность все кейсы покрыть автотестами.
